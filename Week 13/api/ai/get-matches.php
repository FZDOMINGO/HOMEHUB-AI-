...
$stmt = safePrepare($conn, "SELECT * FROM tenant_preferences WHERE tenant_id = ?", "get tenant preferences");
if ($stmt === false) {
    echo json_encode([
        'success' => false,
        'message' => 'Database error occurred.'
...
if ($stmt === false) {
    // If query fails, set matches as empty and continue to simple matching
    $matches = [];
} else {
...
if (empty($matches)) {
    ...
    if ($stmt === false) {
        // If simple matching fails, return empty results
        echo json_encode([
            'success' => true,
            'matches' => [],
            'count' => 0,
            'message' => 'No matches found at this time. Please try adjusting your preferences.'
        ]);
        exit;
    }
