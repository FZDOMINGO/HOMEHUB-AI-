...
try {
    ...
    $notifResult = $conn->query("SELECT COUNT(*) as table_exists FROM information_schema.tables WHERE table_schema = '" . $dbName . "' AND table_name = 'notifications'");
    $notifTableExists = false;
    if ($notifResult) {
        $notifCheck = $notifResult->fetch_assoc();
        $notifTableExists = $notifCheck['table_exists'] > 0;
    }
    ...
    if ($notifTableExists) {
        ...
        ");
        if ($notifData) {
            $activityStats['notifications'] = $notifData->fetch_assoc();
        } else {
    ...
    $viewResult = $conn->query("SELECT COUNT(*) as table_exists FROM information_schema.tables WHERE table_schema = '" . $dbName . "' AND table_name = 'property_views'");
    $viewTableExists = false;
    if ($viewResult) {
        $viewCheck = $viewResult->fetch_assoc();
        $viewTableExists = $viewCheck['table_exists'] > 0;
    }
    ...
    if ($viewTableExists) {
        ...
        ");
        if ($viewData) {
            $activityStats['views'] = $viewData->fetch_assoc();
        } else {
    ...
    // Check for messages
    $msgResult = $conn->query("SELECT COUNT(*) as table_exists FROM information_schema.tables WHERE table_schema = '" . $dbName . "' AND table_name = 'messages'");
    $msgTableExists = false;
    if ($msgResult) {
        $msgCheck = $msgResult->fetch_assoc();
        $msgTableExists = $msgCheck['table_exists'] > 0;
    }
    ...
    if ($msgTableExists) {
        ...
        ");
        if ($msgData) {
            $activityStats['messages'] = $msgData->fetch_assoc();
        } else {
    ...
    $dailyResult = $conn->query("
        ...
        ORDER BY date
    ");
    $dailyActivity = $dailyResult ? $dailyResult->fetch_all(MYSQLI_ASSOC) : [];
